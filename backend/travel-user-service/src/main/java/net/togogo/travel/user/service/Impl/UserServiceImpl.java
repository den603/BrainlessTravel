...
import net.togogo.travel.user.util.HttpUtil;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public String loginByWeChat(String code) {
        try {
            ...
            // 4. 生成 JWT token
            // 【微服务改造】改为携带 userId 签发的重载：网关需要从 token 中直接解析出
            // userId 并写入请求头 X-User-Id 下发给下游服务，而网关没有数据库连接，
            // 无法用 openid 反查用户ID，因此 userId 必须在签发时就写进 token 载荷。
